...
class DICM_Lh_Image extends ET_Builder_Module {
	...
	function get_fields() {
		...
		foreach(range(1,2) as $i) {
			...
			$image_fields['image_' . $i . '_alt'] = array(
				'label'              => esc_html__( 'Alt text' . $i),
				'type'               => 'text',
				'option_category' => 'basic_option',
				'toggle_slug'        => 'main_content',
			);
	...
	function render( $attrs, $content = null, $render_slug ) {
        ...
        <div class="c-links">
            <div class="c-links__content lh-images lh-images--slanted lh-images--<?php echo $icons_length; ?>">
            <?php foreach ($images as $image) { ?>
                <div class="lh-images__item c-links__item ">
                    <img class="lh-image c-links__image" src="<?php echo $image['image'];?>" alt="<?php echo $image['alt'];?>" />
                </div>
            <?php } ?>
            </div>
        </div>
